...
interface IPropertyPremiumCalculator {
...
contract PropertyInsurancePolicy {
    ...
    constructor(address _calculator) {
        Calculator = IPropertyPremiumCalculator(_calculator);
        owner = msg.sender;
    }
    ...
    function generatePremium(
        ...
        newPolicy.propertyType = propertyType;
        newPolicy.age = age;
...
// Helper function to check if an address is already a voter
    ...
    function checkPolicyStatus(address policyHolder, uint256 policyId) public view returns (bool) {
...
// Function to terminate an active policy with a reason
    function terminatePolicy(address policyHolder, string memory reason) public {
        Policy storage policy = policiess[policyHolder][policiesCount[policyHolder]];
...
// Function to remove a voter from the voters array
    ...
    function fileClaim(uint256 _policyId, uint256 _claimAmount, string memory _claimDetails, string[] memory _image) public {
        require(policiesCount[msg.sender] >= _policyId, "Policy ID does not exist");
        ...
        require(lastClaim.status == ClaimStatus.Rejected || lastClaim.policyId == 0, "Existing claim still being processed");
    ...
    enum VoteOption { None, Approve, Reject }
    ...
    event ClaimProcessed(address indexed claimant, uint256 policyId, bool approved, uint256 payout);
    ...
    function voteOnClaim(uint _claimId, VoteOption _vote) public {
        require(policyholders[msg.sender], "Only policyholders can vote on a claim");
        require(_claimId < ClaimId, "Claim does not exist.");
        require(!votes[_claimId][msg.sender].voted, "Already voted on this claim");
        ...
        for (uint256 i = 0; i < totalVoters; i++) {
            if (votes[_claimId][voters[i]].voted) {
                ...
                if (votes[_claimId][voters[i]].option == VoteOption.Approve) {
                    approvals++;
                } else if (votes[_claimId][voters[i]].option == VoteOption.Reject) {
                    rejections++;
                }
            }
        }
    ...
    function finalizeClaim(uint _claimId, bool approved) private {
        ...
        if (approved) {
            ...
            Policy storage policy = policiess[claim.policyholder][claim.policyId];
    ...
    function getVoteCounts(uint256 claimId) public view returns (uint256 approveCount, uint256 rejectCount, uint256 totalVotes) {
    ...
    function drainContract() public onlyOwner {
        ...
        require(contractBalance > 0, "No ether left to drain");
